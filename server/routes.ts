...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get the last uploaded file and auto-load it
app.get("/api/upload/last", async (req, res) => {
  try {
    const lastFile = await storage.getLastUploadedFile();
    if (!lastFile) {
      return res.json({ hasFile: false });
    }

    // Process the file content the same way as upload
    const fileContent = lastFile.fileContent;
    const isJson = lastFile.filename.endsWith('.json') || lastFile.filename.endsWith('.gz');
    
    let players: any[] = [];
    
    if (isJson) {
      let data: any;
      if (lastFile.filename.endsWith('.gz')) {
    ...
    // Process and validate players (same as upload route)
    ...
    for (const player of players) {
      try {
        ...
        if (isJson) {
          ...
          playerData = {
            name: player[0],
            teams: player[1] ? player[1].split(',').map((t: string) => t.trim()) : [],
            years: [],
            achievements: player[2] ? player[2].split(',').map((a: string) => a.trim()) : [],
            careerWinShares: parseInt(player[3]) || 0,
            quality: parseInt(player[4]) || 50
          };
        }
        ...
        errors.push(`Player ${player.name || 'unknown'}: ${error}`);
      }
    }
    ...
    // Calculate career quality scores and clear/add players
    deriveCareerAggregates(validatedPlayers);
    assignQuality(validatedPlayers);
    ...
    // Extract teams and achievements for frontend (same logic as upload)
    const teamNames = Array.from(new Set(createdPlayers.flatMap(p => p.teams)));
    let finalTeamMap = new Map<number, {name: string, abbrev: string, logo?: string}>();
    ...
    res.json({ hasFile: true, data: result });
  } catch (error) {
